    ...
    private function parseResourceXml(xmlString:String, styleSpecMap:Map<String, StyleSpec>, selectorSpecMap:Map<String, SelectorSpec>):Void {
        ...
        for (node in resourceMap) {
            ...
            switch (node.nodeName) {
                ...
                case "drawable": {
                    var hasPackedNode = false;
                    var drawableName = "@drawable/" + name;

                    for (innerNode in node.elements()) {
                        if (hasPackedNode) {
                            throw new Error("Parse error: " + drawableName + " must have exactly one child");
                        }

                        if (innerNode.nodeName != "packed") {
                            throw new Error("Parse error: " + drawableName + " must be packed");
                        }
                        ...
                        if (packedDrawable == null
                            ...
                            throw new Error("Parse error: " + drawableName + " - error in packed node");
                        }

                        if (drawables[packedDrawable] == null) {
                            throw new Error("Parse error: " + drawableName + " - packed drawable not found");
                        }

                        if (drawables[packedDrawable].type != DrawableType.BITMAP) {
                            throw new Error("Parse error: " + drawableName + " - packed drawable must have bitmap type");
                        }

                        drawables[drawableName] = new Drawable(
                            DrawableType.PACKED,
                            drawables[packedDrawable].assetId,
                            packedX,
                            packedY,
                            packedW,
                            packedH
